...
def _gate_soft_penalty_candidates(
    ...
    best_clean_net = max(float(feature.get("net_after_penalty", feature["net"])) for feature in clean)
    best_clean_nph = max(float(feature.get("nph_after_penalty", feature["nph"])) for feature in clean)
    ...
    for feature in features:
        ...
        if penalty <= 0.0 or _soft_penalty_exception_allowed(
            feature,
            best_clean_net,
            best_clean_nph,
            best_clean_score,
...
def _preference_profile_from_entries(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for pref in entries:
        ...
        if ("装货地" in text or "卸货地" in text) and negative_order_text:
        ...
        if "空驶" in text and "总" in text and "公里" in text and ("不得超过" in text or "不超过" in text):
            value = _first_number_before_unit(text, "公里")
            if value:
                if _is_hard_limit_text(text):
        ...
        if coords and ("不同" in text or "自然日" in text) and ("至少" in text or "起码" in text) and "到过" in text:
            need = _number_after_keywords(text, ("至少", "起码"))
            if need:
                lat, lng = coords[0]
                required_location = {
        ...
        if ("不往" in text or "别给我派进" in text or "不进" in text or "避开" in text) and "三月" in text:
            region = _region_from_block_text(text)
            days = _march_days(text)
            if region and days:
                if _is_hard_block_text(text):
        ...
        if not handled:
            hard_rules, soft_rules = _generic_unknown_preference_rules(text, penalty_amount)
...
def _generic_unknown_preference_rules(text: str, penalty_amount: float) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    hard = _looks_like_unknown_hard_rule(text)
    soft = not hard and _looks_like_unknown_soft_rule(text)
    if not hard and not soft:
        return [], []

    rule = _unknown_rule_from_text(text, penalty_amount)
...
def _unknown_rule_from_text(text: str, penalty_amount: float) -> dict[str, Any]:
    rule: dict[str, Any] = {
        "kind": "unknown_text",
        "terms": sorted(_unknown_terms_from_text(text))[:10],
        "days": sorted(_march_days(text)),
        "penalty": penalty_amount,
        "text": text,
    }
    coords = _coordinates(text)
...
def _unknown_terms_from_text(text: str) -> set[str]:
    ...
    patterns = (
        r"(?:不去|不往|不进|不跑|避开|别派|别去|别往|少去|不要去|不要进|不要跑|不接|禁接|推掉|最好别去|尽量不去|尽量别去)([\u4e00-\u9fa5A-Za-z0-9]{2,12})",
...
def _is_hard_block_text(text: str) -> bool:
    if _is_soft_negative_order_text(text):
        return False
    return any(key in text for key in ("别给我派", "查车", "一律", "不得", "不能", "禁止", "不准", "不往", "不进"))
...
def _region_from_block_text(text: str) -> str | None:
    match = re.search(r"(?:不往|不进|不去|派进|避开)([^，。；、]{1,8})(?:跑|那边|查车|地区|$)", text)
    return match.group(1) if match else None
