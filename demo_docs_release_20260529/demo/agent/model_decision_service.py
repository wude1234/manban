    ...
    def _ask_model_for_preference_profile(
        ...
        prompt = {
            ...
            "response_contract": (
                ...
                "Use null instead of guessing numeric limits. Do not copy placeholder zero coordinates from the schema. "
                "scheduled_window means a no-work/rest window when the truck must be stopped, not working hours. "
                "For full-month rules, leave days empty instead of enumerating 0..30. "
                "Do not add unknown_text rules for preferences already represented by category, region, rest, event, or distance fields. "
                "For soft preferences, include the penalty amount."
            ),
        }
        payload = {
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "scheduled_window 只表示必须停车休息/睡觉/不出车的时间段，不要输出可工作时间段。"
                        "无法确定的字段直接省略或设为 null，不要照抄 schema 里的 0 坐标示例。"
                        "整月有效的规则 days 留空，不要枚举 0 到 30。"
                        "已经用 category、region、rest、event、distance 字段表达的规则，不要再重复放进 unknown_text。"
...
def _merge_profiles(local_profile: dict[str, Any], model_profile: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if window is not None:
        ...
        if current_window is None and _window_duration(window) <= 12 * 60:
            out["scheduled_window"] = window

    model_fixed_off_days = _model_days(model_profile.get("fixed_off_days"))
    if out["fixed_off_days"] and model_fixed_off_days:
        out["fixed_off_days"].update(model_fixed_off_days)
    ...
    for item in _as_list(model_profile.get("blocked_region_days") or model_profile.get("forbidden_region_days")):
        ...
        if hard:
            if len(days) >= 25:
                out["forbidden_regions"].add(region)
            elif region in out["blocked_region_days"]:
                continue
            else:
    ...
    if home_deadline is not None and isinstance(out.get("home_deadline"), dict):
        out["home_deadline"] = {**home_deadline, **out["home_deadline"]}
    ...
    out["soft_penalty_rules"] = _filter_redundant_soft_unknown_rules(
        _dedupe_soft_penalty_rules(out["soft_penalty_rules"]),
        out,
    )
    out["unknown_hard_rules"] = _filter_redundant_unknown_rules(_dedupe_unknown_rules(out["unknown_hard_rules"]), out)
...
def _model_required_region(value: Any) -> dict[str, Any] | None:
    ...
    if lat is not None and lng is not None and _valid_model_lat_lng(lat, lng):
        out["lat"] = lat
        out["lng"] = lng
...
def _model_required_location(value: Any) -> dict[str, Any] | None:
    ...
    lng = _float_or_none(value.get("lng"))
    min_days = _int_or_none(value.get("min_days") or value.get("days_required") or value.get("need_days")) or 0
    if lat is None or lng is None or not _valid_model_lat_lng(lat, lng) or min_days <= 0:
        return None
    return {
...
def _model_box(value: Any) -> dict[str, float] | None:
    ...
    if None in (lat_min, lat_max, lng_min, lng_max):
        return None
    if not (
        _valid_model_lat_lng(float(lat_min), float(lng_min))
        and _valid_model_lat_lng(float(lat_max), float(lng_max))
    ):
        return None
    if abs(float(lat_max) - float(lat_min)) < 1e-6 or abs(float(lng_max) - float(lng_min)) < 1e-6:
        return None
    return {
...
def _model_target(value: Any) -> dict[str, float] | None:
    ...
    lat = _float_or_none(value.get("lat"))
    lng = _float_or_none(value.get("lng"))
    if lat is None or lng is None or not _valid_model_lat_lng(lat, lng):
        return None
    return {"lat": lat, "lng": lng}


def _valid_model_lat_lng(lat: float, lng: float) -> bool:
    return 15.0 <= float(lat) <= 30.0 and 105.0 <= float(lng) <= 125.0
...
def _filter_redundant_soft_unknown_rules(
    ...
    for rule in rules:
        if not isinstance(rule, dict):
            continue
        if str(rule.get("kind", "")) == "unknown_text" and _unknown_rule_redundant_with_profile(rule, profile):
            continue
        out.append(rule)
    return out


def _filter_redundant_unknown_rules(
    rules: list[dict[str, Any]],
    profile: dict[str, Any],
) -> list[dict[str, Any]]:
    return [rule for rule in rules if not _unknown_rule_redundant_with_profile(rule, profile)]


def _unknown_rule_redundant_with_profile(rule: dict[str, Any], profile: dict[str, Any]) -> bool:
    if rule.get("coords") or rule.get("pickup_max_km") is not None or rule.get("haul_max_km") is not None:
        return False
    terms = [str(term).strip() for term in rule.get("terms", []) or [] if str(term).strip()]
    if not terms:
        return True
    covered = set()
    covered.update(str(item) for item in profile.get("forbidden_categories", set()) or set())
    covered.update(str(item) for item in profile.get("preferred_categories", set()) or set())
    covered.update(str(item) for item in profile.get("forbidden_regions", set()) or set())
    for soft_rule in profile.get("soft_penalty_rules", []) or []:
        if not isinstance(soft_rule, dict) or soft_rule is rule:
            continue
        for key in ("category", "region"):
            if soft_rule.get(key):
                covered.add(str(soft_rule[key]))
    actionable = []
    for term in terms:
        if _non_actionable_unknown_term(term):
            continue
        if any(term == item or term in item or item in term for item in covered if item):
            continue
        actionable.append(term)
    return not actionable


def _non_actionable_unknown_term(term: str) -> bool:
    return any(
        key in term
        for key in (
            "休息",
