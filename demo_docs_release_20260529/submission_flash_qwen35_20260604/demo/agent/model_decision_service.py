...
def _planned_off_days(profile: dict[str, Any]) -> set[int]:
    required = int(profile.get("off_days_required", 0) or 0)
    if required <= 0:
        return set()
    blocked: set[int] = set()
...
def _region_from_forbidden_text(text: str) -> str | None:
    ...
    if quoted:
        return quoted[0]
    match = re.search(r"在([^，。；、]{1,8})的货", text)
...
def _parse_scheduled_window(text: str) -> tuple[int, int] | None:
    ...
    if "零点" in text and ("六点" in text or "6点" in text):
        return (0, 6 * 60)
    match = re.search(
        r"([0-9一二三四五六七八九十两零]+)\s*点[^0-9一二三四五六七八九十两零]{0,6}(?:次日|早上|凌晨)?\s*([0-9一二三四五六七八九十两零]+)\s*点",
        text,
    )
    if not match:
        return None
    start, end = _parse_small_int(match.group(1)), _parse_small_int(match.group(2))
...
def _march_days(text: str) -> set[int]:
    if "三月" not in text:
        return set()
    days = set()
    for raw in re.findall(r"([0-9一二三四五六七八九十两]+)\s*号", text):
