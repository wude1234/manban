...
```text
hot_v60_d004_step7_dg_plus_step93
score = 312269.66
penalty = 12265
```
...
v60 的关键新增规律来自 D004 早期主动空驶：step7 原策略接短距离货 `1677`，但完整尾部回放证明主动空驶到东莞附近能打开更高价值链，尽管多付 `400` 偏好罚分，总分仍净增 `+581.84`。这说明区域价值不是单独强弱标签，而是“当前动作是否把司机送进后续货源链”的状态价值。

如果后续 D001/D009/D006 等低分或高罚分司机继续挖掘跑出更高组合，再替换提交底座；否则 v60 作为当前提交候选。
