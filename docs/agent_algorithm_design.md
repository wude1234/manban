...
profile = hot_v77_d004_triple_469204_299927_303849
base = v30 driver-specific planner
upgrade = v32-v77 counterfactual memory + phase action gates + low-efficiency route repair + exact sequence probing + wait-as-route-plan teacher + schedule-aware value teacher + two-step/three-step route rebase + clean harness ablation + D010 active reposition and mutable month-end sequence teacher + D004 triple route teacher
```
...
- v76 证明“正样本记忆”不能僵化。继续加入 step106 `cargo205150` 后，D010 step103 改回 `cargo196038` 的旧目的地反而更优，形成 `196038 -> 484175 -> 205150` 的尾链，总分提升到 `314529.94`。这说明 Agent memory 应存储可重估的 route teacher，而不是不可撤销的预录答案。
- v77 证明三步 Route Plan 能继续突破平台。D004 在 step93/94/96 执行 `cargo469204 -> cargo299927 -> cargo303849`，D004 净收益 `39325.91 -> 39516.78`，总分 `314720.81`。这不是更强区域的静态偏好，而是一个完整后继链替换；实现上还暴露了两个 Agent 安全层问题：旧 cargo switch 必须允许 route-level teacher 覆盖，phase guard 必须按 query 后 action_start 判断。
